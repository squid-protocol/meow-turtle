...
# --- UI LAYOUT ---
with ui.card().classes('w-full h-screen bg-red-900 flex flex-col items-center justify-center p-8 gap-4'):
    ...
    # Log Viewer
    with ui.expansion('View Crash Logs', icon='article').classes('w-full max-w-2xl bg-red-800 text-white'):
        log_text = "".join(read_last_logs())
        ui.code(log_text).classes('w-full h-64 text-xs')

    # Actions
    with ui.row().classes('gap-4 mt-8'):
        ui.button('RESTART APP', on_click=restart_system).props('color=white text-color=red-900 size=lg')
        ui.button('REBOOT PI', on_click=reboot_pi).props('color=red-700 text-color=white size=lg')
