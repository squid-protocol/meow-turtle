...
def ping_node(node_id, baud_rate, timeout=1.0):
    ...
    try:
        ...
        pkt = meowprotocol.build_packet(node_id, 0, 1, meowprotocol.MSG_TYPE_CMD, "PING")
...
def shift_baud_ram(node_id, current_baud, target_baud):
    ...
    try:
        ...
        pkt = meowprotocol.build_packet(node_id, 0, 255, MSG_TYPE_CMD_BAUD, str(target_baud))
        ser.write(pkt)
        ...
        print(f"  [i] Shift packet transmitted.")
        return True
...
def test_link(node_id, baud_rate):
    ...
    for seq in range(1, PACKETS_PER_TEST + 1):
        pkt = meowprotocol.build_packet(node_id, 0, seq % 255, meowprotocol.MSG_TYPE_CMD, "PING")
        t_start = time.time()
        ser.write(pkt)
        ...
        if response_found: sys.stdout.write("=")
    ...
    print(f"  [i] Test Complete. Hit Rate: {success_rate}%, CRC Errors: {parser.crc_error_count}")
...
def rubber_band_recovery(node_id):
    ...
    print(f"  [!!!] Node is deaf or lost. Cannot send a RAM shift command.")
    print("  [i] Waiting 65 seconds for the Pico's Autonomous Network Watchdog to bite...")
...
if __name__ == "__main__":
    ...
    for target_baud in TEST_BAUDS:
        ...
        if target_baud != SAFE_BAUD:
            ...
            if ping_node(node, SAFE_BAUD, timeout=0.5):
                print("[!] ALIVE")
                print("  [✗] FATAL: Pico ignored the shift command! It is still at the old baud.")
                print("  [i] The shift packet was likely corrupted by EMI. Aborting this frequency.")
            ...
            if ping_node(node, target_baud, timeout=0.5):
                ...
                print("[✗] DEAF")
                print("  [✗] Node left the old baud but is completely deaf at the new one.")
    ...
    print("="*60)
    print(f"| {'Baud Rate':<10} | {'Success':<8} | {'Avg Lat':<8} | {'Max Lat':<8} | {'CRC Errs':<8} |")
    print("|" + "-"*12 + "|" + "-"*10 + "|" + "-"*10 + "|" + "-"*10 + "|" + "-"*10 + "|")
    
    for r in results:
        print(f"| {r['baud']:<10} | {r['success']:>6.1f} % | {r['avg_lat']:>5.1f} ms | {r['max_lat']:>5.1f} ms | {r['crc']:>8} |")
    print("="*60 + "\n")