...
def memory_guard(threshold_bytes=8192):
    ...
    if free < threshold_bytes:
        log.warn(
            "SYS", f"Memory Guard Triggered: {free} bytes free (<{threshold_bytes})"
        )
        return False
    return True
...
def send_reliable(uart, target, m_type, payload):
    ...
    with state_lock:
        ...
        if shared_state["next_seq"] == 0:
            shared_state["next_seq"] = 1
...
def core1_task():
    ...
    beam_gate_sigma = system_config.get("tuning", {}).get(
        "beam_downward_gate_sigma", 1.5
    )
                    ...
                    if tsl_val is not None and tsl_val > 0:
                        ...
                        if is_calibrating:
                            ...
                            if len(current_samples) >= 20:
                                ...
                                variance = (
                                    sum((x - beam_mean) ** 2 for x in current_samples)
                                    / 20.0
                                )
                                beam_stddev = variance**0.5

                                # Apply Noise Floor Trap (Force at least 5% of mean for high-gain stability)
                                dynamic_noise_floor = max(
                                    beam_stddev, beam_min_sigma, beam_mean * 0.05
                                )

                                # Calculate Final Trip Threshold
                                beam_threshold = beam_mean - (
                                    beam_trip_sigma * dynamic_noise_floor
                                )

                                log.info(
                                    "SEN",
                                    f"Beam Calibrated. Mean: {int(beam_mean)}, StdDev: {int(beam_stddev)}, Thresh: {int(beam_threshold)}",
                                )

                                # Export to Sensor Manager for GUI Telemetry
                                with sns_mgr.lock:
                                    sns_mgr.last_results["BEAM_MEAN"] = int(beam_mean)
                                    sns_mgr.last_results["BEAM_THRESH"] = int(
                                        beam_threshold
                                    )
                        ...
                        # 2. Active Run Phase
                            ...
                            beam_gate_floor = beam_mean - (
                                beam_gate_sigma * dynamic_noise_floor
                            )
                            beam_threshold = beam_mean - (
                                beam_trip_sigma * dynamic_noise_floor
                            )
                            ...
                            if tsl_val > beam_mean or tsl_val >= beam_gate_floor:
                                beam_mean = (tsl_val * beam_alpha) + (
                                    beam_mean * (1.0 - beam_alpha)
                                )

                                if sns_mgr:
                                    with sns_mgr.lock:
                                        sns_mgr.last_results["BEAM_MEAN"] = int(
                                            beam_mean
                                        )
                                        sns_mgr.last_results["BEAM_THRESH"] = int(
                                            beam_threshold
                            ...
                            if beam_break_start_ms > 0:
                                if (
                                    time.ticks_diff(
                                        time.ticks_ms(), beam_break_start_ms
                                    )
                                    >= beam_sustain_ms
                                ):
                                    is_broken = True
                            ...
                            with state_lock:
                                ...
                                if is_broken and not was_broken:
                                    ...
                                    shared_state["beam_start_pulse"] = sensor_data.get(
                                        "global_tick_TOTAL", 0
                                    )
                                ...
                                # RISING EDGE (Part Exits Beam)
                                    ...
                                    log.info(
                                        "SEN",
                                        f"PART MEASURED! CenterPulse: {center_pulse}, Duration: {length_ms}ms",
                                    )
    ...
    try:
        ...
        if log_traffic:
            try:
                ...
                p_load = "BIN"
            log.debug(
                "NET", f"[RX] T:{target} S:{source} Q:{seq} TY:{p_hex} P:{p_load}"
            )
        ...
        if last_seq == seq:
            if log_traffic:
                log.debug("NET", f"[DEDUPE] Dropping Seq {seq} from {source}")
            return meowprotocol.build_packet(
                source, shared_state["id"], seq, meowprotocol.MSG_TYPE_ACK, str(seq)
            )
        ...
        try:
            ...
            if m_type == meowprotocol.MSG_TYPE_CMD_STS:
                ...
                response = meowprotocol.build_packet(
                    source,
                    shared_state["id"],
                    seq,
                    meowprotocol.MSG_TYPE_STS,
                    status_str,
                )
                ...
                if cmd_str == "IDLE":
                    ...
                    if shared_state["current_state"] == STATE_ERROR:
                        response = meowprotocol.build_packet(
                            ...
                            shared_state["id"],
                            seq,
                            meowprotocol.MSG_TYPE_NAK_STATE,
                            "MUST_RESET",
                        )
                    else:
                        with state_lock:
                            shared_state["current_state"] = STATE_FLOW
                        response = meowprotocol.build_packet(
                            source,
                            shared_state["id"],
                            seq,
            ...
            # --- QUERY ACTUATORS - 0x15 [Spec 4.3.8] ---
                ...
                if act_mgr:
                    ...
                    response = meowprotocol.build_packet(
                        source, shared_state["id"], seq, meowprotocol.MSG_TYPE_ACT, data
                    )
            ...
            # --- QUERY SENSORS - 0x16 [Spec 4.3.9] ---
                ...
                if sns_mgr:
                    if hasattr(sns_mgr, "get_telemetry_string"):
                        ...
                        data = "SNS_OK"
                    response = meowprotocol.build_packet(
                        source, shared_state["id"], seq, meowprotocol.MSG_TYPE_SNS, data
                    )
            ...
            # --- SYSTEM MANIFEST - 0x12 [Spec 12.4.3] ---
                ...
                if sys_man:
                    ...
                    data = "SYS_MAN=FAIL"
                response = meowprotocol.build_packet(
                    source, shared_state["id"], seq, meowprotocol.MSG_TYPE_VRS, data
                )
            ...
            # --- QUERY CONFIG - 0x17 [Spec 4.3.11.3] ---
                ...
                try:
                    ...
                    response = meowprotocol.build_packet(
                        ...
                        shared_state["id"],
                        seq,
                        meowprotocol.MSG_TYPE_NAK_SYNTAX,
                        "JSON_ERR",
                    )
            ...
            # --- LIVE TUNING - 0x18 [Spec 4.3.10] ---
                ...
                response = meowprotocol.build_packet(
                    source, shared_state["id"], seq, response_type, response_payload
                )

            # --- OTA START - 0x50 [Spec 4.3.14.4] ---
            elif m_type == meowprotocol.MSG_TYPE_OTA_START:
                with state_lock:
                    shared_state["current_state"] = STATE_OTA
                if act_mgr:
                    act_mgr.emergency_stop()
            ...
            # --- OTA DATA - 0x51 [Spec 4.3.14.3.1] ---
                ...
                if shared_state["current_state"] == STATE_OTA:
                    with state_lock:
                        shared_state["ota_last_activity"] = time.ticks_ms()
                    ...
                    response = meowprotocol.build_packet(
                        ...
                        shared_state["id"],
                        seq,
                        meowprotocol.MSG_TYPE_ACK if ok else meowprotocol.MSG_TYPE_NAK,
                        msg,
                    )
                else:
                    response = meowprotocol.build_packet(
            ...
            # --- TIME SYNC - 0x1A [Spec 4.3.13] ---
            elif m_type == 0x1A:  # MSG_TYPE_CMD_SYNC_TIME
                # Acknowledge the time sync so Core 0 doesn't timeout
                response = meowprotocol.build_packet(
                    source, shared_state["id"], seq, meowprotocol.MSG_TYPE_ACK, str(seq)
                )

            # --- RAM-ONLY BAUD SHIFT - 0x1B [Spec 4.3.14] ---
            elif m_type == 0x1B:  # MSG_TYPE_CMD_BAUD
                try:
                    new_baud = int(payload.decode("ascii"))

                    # 1. Build and send the ACK at the CURRENT (old) speed
                    ack_pkt = meowprotocol.build_packet(
                        source,
                        shared_state["id"],
                        seq,
                        meowprotocol.MSG_TYPE_ACK,
                    ...
                    # 2. Give the wire 50ms to physically transmit the bytes before the clock changes
                    time.sleep(0.05)

                    # 3. Shift the hardware UART instantly
                    uart.init(baudrate=new_baud)
                    log.warn("NET", f"Hardware shifted to {new_baud} bps (RAM Only)")

                    # Prevent the outer function from returning a second response
                    response = None
                except Exception:
                    response = meowprotocol.build_packet(
                        source,
            ...
            response = meowprotocol.build_packet(
                source, shared_state["id"], seq, meowprotocol.MSG_TYPE_NAK_SYNTAX, "ERR"
            )
...
def main():
    ...
    log.info(
        "APP-DIAG",
        f"Debug Settings -> Health:{test_cfg.get('health_print_interval_ms')}ms, Traffic:{log_traffic}",
    )

    # 3. Init Subsystems (PARANOID ISOLATION & TRACEBACK CATCHER)
    ...
    try:
        ...
        with open("crash.log", "w") as f:
            f.write(f"ACTUATOR FAIL:\n{buf.getvalue()}")
        log.crit("SYS", f"Actuator Init Fail: {e}")
        boot_errors.append(f"ACT_FAIL: {e}")
    ...
    try:
        ...
        sys.print_exception(e, buf)
        with open("crash.log", "w") as f:
            f.write(f"SENSOR FAIL:\n{buf.getvalue()}")
        log.crit("SYS", f"Sensor Init Fail: {e}")
        boot_errors.append(f"SNS_FAIL: {e}")

    try:
        ota_mgr = ota.OTAManager()
    except Exception as e:
        log.crit("SYS", f"OTA Init Fail: {e}")
        boot_errors.append(f"OTA_FAIL: {e}")

    try:
        sys_man = diagnostics.SystemManifest()
    except Exception as e:
        log.crit("SYS", f"Manifest Init Fail: {e}")
        boot_errors.append(f"MAN_FAIL: {e}")
    ...
    # --- PHASE 2: DEGRADED BOOT ALARM ---
    if boot_errors:
        with state_lock:
            shared_state["current_state"] = STATE_ERROR
            shared_state["error_code"] = str(boot_errors[0])[:32]
        log.crit("SYS", "BOOT DEGRADED - Entering ERROR State")
    else:
        with state_lock:
            shared_state["current_state"] = STATE_IDLE
    ...
    last_valid_rx_time = time.ticks_ms()  # NEW: Network Watchdog Timer
    ...
    while True:
        try:
            ...
            with state_lock:
                ...
                if curr_state == STATE_FLOW:
                    blink_rate = 250  # FLOW (2Hz = 250ms toggle)
                elif curr_state == STATE_ERROR:
                    blink_rate = 100  # ERROR (5Hz = 100ms toggle)
                elif curr_state == STATE_OTA:
                    blink_rate = 100
            ...
            if (
                shared_state["current_state"] == STATE_ERROR
                and shared_state["error_code"] != "NONE"
            ):
                if shared_state["error_code"] != last_error_code_sent:
                    send_reliable(
                        uart, 0, meowprotocol.MSG_TYPE_ALARM, shared_state["error_code"]
                    )
                    last_error_code_sent = shared_state["error_code"]
                    if log_traffic:
                        log.debug("NET", f"[TX ALARM] {shared_state['error_code']}")
            ...
            if send_evt:
                ...
                if log_traffic:
                    log.debug("NET", f"[TX EVT] {payload}")
                ...
                for seq_id, item in items:
                    if time.ticks_diff(t_ms, item["ts"]) > 200:
                        if item["retries"] < max_retries:
                            ...
                            if log_traffic:
                                log.debug("NET", f"[TX-RETRY] {queue_name} ID:{seq_id}")
                        else:
                            log.crit(
                                "NET", f"Max Retries Exceeded {queue_name} ID {seq_id}"
                            )
            ...
            # --- TRAFFIC GATING ---
            queues_empty = (
                len(shared_state["tx_safety"]) == 0
                and len(shared_state["tx_events"]) == 0
            )
            ...
            if True:
                ...
                if (
                    s_int > 0
                    and sns_mgr
                    and time.ticks_diff(t_ms, last_sensor_print) > s_int
                ):
                    if hasattr(sns_mgr, "get_telemetry_string"):
                ...
                if (
                    a_int > 0
                    and act_mgr
                    and time.ticks_diff(t_ms, last_act_print) > a_int
                ):
                    if hasattr(act_mgr, "current"):
                        log.debug(
                            "DBG-ACT",
                            f"Targets: {act_mgr.targets} Current: {act_mgr.current}",
                        )
                    else:
            ...
            if incoming_batch:
                ...
                def get_prio(p):
                    mt = p[3]
                    if (
                        mt == meowprotocol.MSG_TYPE_CMD_STOP
                        or mt == meowprotocol.MSG_TYPE_ALARM
                    ):
                        return 0  # Critical
                    if mt == meowprotocol.MSG_TYPE_CMD:
                        return 1  # Command
                    return 2  # Bulk Data
                ...
                for pkt in incoming_batch:
                    last_valid_rx_time = t_ms  # NEW: Feed the Network Watchdog
            ...
            # --- NEW: AUTONOMOUS NETWORK WATCHDOG ---
            # If we haven't received a valid packet in 60 seconds, we are deaf.
            # Intentionally crash the loop so boot.py deducts a life.
            if test_cfg.get("network_watchdog_enabled", True) and not test_cfg.get(
                "standalone_mode", False
            ):
                if time.ticks_diff(t_ms, last_valid_rx_time) > 60000:
                    raise Exception("Network Timeout - Deaf to RS485")
