...
class IndustrialMapper(ast.NodeVisitor):
    ...
    def get_summary(self, node):
        ...
        if doc:
            ...
            spec_str = (
                ", ".join([f"S{s}" for s in unique_specs]) if unique_specs else ""
            )
...
def run_audit():
    ...
    for path in sorted(paths):
        if path == "project_mapper.py" or any(
            x in path for x in ["original", "initial", "backup"]
        ):
            continue
        ...
        try:
            ...
            for res in visitor.results:
                ...
                print(
                    f"{fname:<20} | {display_name:<35} | {res['specs']:<12} | {res['lines']:<5} | {density:<17} | {res['summary']}"
                )
                report_content.append(
                    f"| {fname} | {display_name} | {res['specs']} | {res['lines']} | {res['summary']} |"
                )
                first_row = False
            if visitor.results:
                print("-" * 165)
    ...
    print("Report generated: SYSTEM_MANIFEST.md")
