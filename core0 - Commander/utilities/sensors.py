...
class PulseCounter:
        ...
        try:
            if mirror_pin_num is not None:
                ...
                self.sm = rp2.StateMachine(
                    state_machine_id,
                    pio_programs.pulse_counter_with_mirror,
                    in_base=self.pin,
                    sideset_base=self.mirror_pin,
                )
            else:
                self.sm = rp2.StateMachine(
                    state_machine_id,
                    pio_programs.pulse_counter_simple,
                    in_base=self.pin,
                )
    ...
    def get_data(self):
        ...
        """
        if not self.sm:
            return ERR_POISONED, ERR_POISONED
...
class BusWrapper:
    ...
    def __init__(self, config):
        ...
        self.err_count = 0
        self.last_read_ms = 0  # --- I2C THROTTLE TIMER ---
        self.ema_state = {}  # [LEVEL 2] EMA Filter History
        self.setup()
    ...
    def setup(self):
        ...
        try:
            # Dropped I2C speed to 100kHz to fix jumper wire capacitance/byte tearing
            ...
            devs = self.i2c.scan()
            log.info(
                "SEN", f"Bus {self.id} ({self.prefix}) Scan: {[hex(x) for x in devs]}"
            )
            ...
            if 0x29 in devs:
                ...
                if tsl2591:
                    try:
                        self.sensors[name] = tsl2591.TSL2591(self.i2c)
                    except:
                        self.sensors[name] = ERR_BUG
                else:
                    self.sensors[name] = ERR_MISSING
            ...
            if 0x68 in devs:
                ...
                if mpu6050:
                    try:
                        self.sensors[name] = mpu6050.MPU6050(self.i2c)
                    except:
                        self.sensors[name] = ERR_BUG
                else:
                    self.sensors[name] = ERR_MISSING
    ...
    def read(self, result_dict):
        ...
        """
        # --- I2C BUS THROTTLE (20Hz) ---
        # Prevents ultra-fast Core 1 from hammering the slow I2C silicon
        now = time.ticks_ms()
        if time.ticks_diff(now, self.last_read_ms) < 50:
            return False  # Skip read, let previous values persist
            ...
            try:
                ...
                if "TSL" in name:
                    ...
                    if isinstance(vals, dict):
                        # [FIX] Clear any old I/O error flags now that the sensor is healthy!
                        ...
                        for k, v in vals.items():
                            ...
                            # --- LEVEL 2: EMA FILTER ---
                            alpha = 0.3  # 30% New Data, 70% Old Data (Shock Absorber)
                            if axis_name not in self.ema_state:
                                self.ema_state[axis_name] = v
                            else:
                                self.ema_state[axis_name] = (v * alpha) + (
                                    self.ema_state[axis_name] * (1.0 - alpha)
                                )

                            result_dict[axis_name] = self._apply_filters(
                                axis_name, round(self.ema_state[axis_name], 2)
                            )
                        continue
                    else:
                        val = vals
...
class SensorManager:
    ...
    def __init__(self, config):
        ...
        log.info("SEN", "SensorManager v1.10 Startup...")
        ...
        for d_cfg in self.config.get("digital_inputs", []):
            try:
                d = DigitalInput(
                    d_cfg["pin"],
                    d_cfg.get("invert", False),
                    d_cfg.get("debounce_ms", 50),
                )
    ...
    def read_all(self):
        ...
        with self.lock:
            ...
            for name, ctr in self.counters.items():
                ...
                if total != ERR_POISONED:
                    self.last_results[f"{name}_TOTAL"] = total
            ...
            for bus in self.buses:
                ...
                if io_err:
                    ...
                    if bus.err_count > 10:
                        log.warn(
                            "SEN", f"Bus {bus.id} Lock Detected (-5). Resetting..."
                        )
                        bus.teardown()
                        time.sleep_ms(50)
                        bus.setup()
                        ...
                        for k in bus.sensors.keys():
                            self.last_results[k] = ERR_BUS_LOCK
                else:
                    bus.err_count = 0
