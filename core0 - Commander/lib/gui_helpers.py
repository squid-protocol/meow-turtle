...
class LogManager:
    ...
    def get_signature(self, tag, msg):
        ...
        """
        if "Packet lost" in msg:
            return f"{tag}|PKT_LOST"
        if "SEQ SKIP" in msg:
            return f"{tag}|SEQ_SKIP"
        if "CRC ERROR" in msg:
            return f"{tag}|CRC_ERR"
        return f"{tag}|{msg}"
...
class GUIModals:
    ...
    def open_inspector(pid):
        ...
        with (
            ui.dialog() as dialog,
            ui.card().classes(
                "bg-slate-900 border border-slate-700 w-[500px] p-0 overflow-hidden"
            ),
        ):
            with ui.row().classes(
                "w-full bg-slate-950 p-2 border-b border-slate-800 items-center justify-between"
            ):
                ui.label(f"HARDWARE INSPECTOR :: PICO {pid}").classes(
                    "text-xs font-black text-cyan-400 tracking-widest"
                )
                ui.button(icon="close", on_click=dialog.close).props(
                    "flat dense size=sm color=slate-500"
                )

            with ui.column().classes("p-4 w-full gap-4 max-h-[80vh] overflow-y-auto"):
                # 1. Identity Block
                with ui.column().classes("gap-1"):
                    ui.label("SYSTEM IDENTITY").classes(
                        "text-[10px] font-black text-slate-500"
                    )
                    with ui.grid(columns=2).classes(
                        "w-full gap-x-4 gap-y-1 bg-black/20 p-2 rounded"
                    ):
                ...
                v_box = ui.column().classes(
                    "w-full bg-black/40 p-2 rounded border border-slate-800 max-h-[150px] overflow-y-auto gap-0.5"
                )
                versions = getattr(limb, "remote_versions", {})
                with v_box:
                    if not versions:
                        ui.label("No manifest loaded.").classes(
                            "text-[10px] italic text-slate-600"
                        )
                    for k, v in sorted(versions.items()):
                        with ui.row().classes("w-full justify-between no-wrap"):
                            ui.label(k).classes("text-[10px] font-bold text-slate-500")
                            ui.label(str(v)).classes(
                                "text-[10px] font-mono text-cyan-500"
                            )

                # 3. Runtime Configuration Block
                ui.label("RUNTIME CONFIGURATION").classes(
                    "text-[10px] font-black text-slate-500"
                )
                c_box = ui.column().classes(
                    "w-full bg-black/40 p-2 rounded border border-slate-800 max-h-[200px] overflow-y-auto gap-1"
                )
                configs = getattr(limb, "remote_config", {})
                with c_box:
                    if not configs:
                        ui.label("No config loaded.").classes(
                            "text-[10px] italic text-slate-600"
                        )
                    for k, v in sorted(configs.items()):
                        with ui.row().classes("w-full gap-2 items-start h-auto"):
                            ...
                            ui.label(val_str).classes(
                                "text-[11px] font-mono text-amber-400 break-all"
                            )

            with ui.row().classes(
                "w-full p-2 bg-slate-950 border-t border-slate-800 justify-end gap-2"
            ):

                async def refresh_data():
                    await GLOBAL_TWIN.coordinator.send_cmd("FETCH_VERSIONS")
                    await GLOBAL_TWIN.coordinator.send_cmd("FETCH_CONFIG")
                    GLOBAL_TWIN.coordinator._notify(
                        "Data Requested. Close & Reopen this window in 2 seconds.",
                        type="info",
                    )

                ui.button("REFRESH ALL", on_click=refresh_data).props(
                    "outline dense size=sm color=cyan"
                )
            dialog.open()
    ...
    def open_calibration_wizard(coord):
        ...
        if GLOBAL_TWIN.host_state != ms.STATE_DEV:
            coord._notify("ENABLE DEV MODE FIRST", type="negative")
            return

        with (
            ui.dialog() as dialog,
            ui.card().classes(
                "w-[450px] bg-slate-900 border border-slate-700 p-0 overflow-hidden"
            ),
        ):
            ...
            async def toggle_test():
                ...
                if not wizard["running"]:
                    ...
                    wizard["running"] = True
                    await coord.fleet.send(
                        3, meowprotocol.MSG_TYPE_CMD, "SET:conveyor=0.75"
                    )
                    btn_run.props("color=red icon=stop").set_text("STOP TEST RUN")
                    lbl_status.set_text("MOTOR RUNNING - CAPTURING PULSES...").classes(
                        "text-emerald-400"
                    )
                else:
                    ...
                    await coord.fleet.send(
                        3, meowprotocol.MSG_TYPE_CMD, "SET:conveyor=0.0"
                    )
                    wizard["delta_p"] = abs(get_current_pulses() - wizard["start_p"])
                    btn_run.disable()
                    lbl_status.set_text(
                        f"TEST COMPLETE: {wizard['delta_p']} Pulses Captured"
                    ).classes("text-white")
            ...
            def finalize():
                ...
                try:
                    ...
                    if dist > 0 and wizard["delta_p"] > 0:
                        ...
                        coord._notify(
                            f"Calibration Saved: {dist / wizard['delta_p']:.5f} mm/p",
                            type="positive",
                        )
                        dialog.close()
                except:
                    coord._notify("Invalid Distance Input", type="negative")

            # UI Component Layout
            with ui.row().classes(
                "w-full bg-slate-950 p-3 border-b border-slate-800 items-center justify-between"
            ):
                ui.label("CONVEYOR SPATIAL CALIBRATION").classes(
                    "text-sm font-black text-cyan-400"
                )
                ui.button(icon="close", on_click=dialog.close).props(
                    "flat dense color=slate-500"
                )

            with ui.column().classes("p-4 w-full gap-4"):
                ui.label(
                    "This wizard determines the exact movement ratio by running the belt and measuring physical distance."
                ).classes("text-xs text-slate-400 italic")

                btn_run = ui.button(
                    "START CALIBRATION RUN", on_click=toggle_test
                ).classes("w-full py-4 text-lg font-black")
                lbl_status = ui.label("Ready to begin.").classes(
                    "text-center w-full font-mono text-slate-500"
                )

                input_col = ui.column().classes(
                    "w-full hidden bg-black/40 p-4 rounded border border-slate-800 gap-3"
                )
                with input_col:
                    ui.label("STEP 2: Enter measured distance").classes(
                        "text-[10px] font-bold text-amber-500"
                    )
                    dist_input = ui.input("Physical Distance (mm)").props(
                        'type=number outlined dark dense suffix="mm"'
                    )
                    ui.button("CALCULATE & COMMIT TO DISK", on_click=finalize).props(
                        "color=cyan"
            ...
            dialog.on(
                ...
                lambda: asyncio.create_task(
                    coord.fleet.send(3, meowprotocol.MSG_TYPE_CMD, "SET:conveyor=0.0")
                ),
            )
            dialog.open()
