...
class SystemCoordinator:
    ...
    async def start(self):
        ...
        try:
            ...
            self.fleet = operator(
                self.config_ports, self.logic_queue, self.warning_queue, self
            )
            ...
            if not ready:
                logger.warning(
                    "[Hub] Bootstrap: Some limbs failed to report ready state."
                )
        ...
        while self.running:
            try:
                ...
                for pid in self.rtt_pending:
                    self.rtt_pending[pid] = {
                        s: t for s, t in self.rtt_pending[pid].items() if now - t < 5.0
                    }
    ...
    async def send_physical(self, pid, msg_type, payload):
        ...
        if seq_id is not None and msg_type in [
            meowprotocol.MSG_TYPE_CMD,
            meowprotocol.MSG_TYPE_SET_CFG,
            meowprotocol.MSG_TYPE_CMD_RST,
        ]:
    ...
    async def send_manual_command(self, pid, act_name, val):
        ...
        if norm_name.lower().startswith("sol") and float(val) > 0.0:
            ...
            async def safe_pulse():
                await self.send_physical(
                    pid, meowprotocol.MSG_TYPE_CMD, f"ACT:{norm_name}={val}"
                )
                await asyncio.sleep(0.5)  # 500ms safety pulse
                await self.send_physical(
                    pid, meowprotocol.MSG_TYPE_CMD, f"ACT:{norm_name}=0.0"
                )
        ...
        self.cmd_dedupe_cache[cache_key] = {
            "payload": payload,
            "ts": now,
            "mtype": meowprotocol.MSG_TYPE_CMD,
        }
    ...
    def _normalize_actuator_name(self, pid, name):
        ...
        limb = GLOBAL_TWIN.limbs.get(pid)
        if not limb:
            return name  # Trust fallback
        if name in limb.actuators:
            return name
                ...
                for pid, start in list(self.pending_log_requests.items()):
                    if now - start > 3.0:
                        ...
                        if not self.pending_log_requests:
                            self._notify(
                                "Log sweep complete. No new crash logs found.",
                                type="positive",
                            )
                await asyncio.sleep(0.5)
            except asyncio.CancelledError:
