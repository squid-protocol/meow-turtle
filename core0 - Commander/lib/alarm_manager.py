...
class AlarmManager:
        ...
        for limb_id, limb in self.model.limbs.items():
            ...
            if limb.remote_state == ms.STATE_OFFLINE:
                if elapsed_since_boot > self.LIMITS["MATURITY_TIME_S"]:
                    self.raise_alarm(f"L{limb_id}_OFFLINE", ms.SEVERITY_CRITICAL, "Heartbeat Lost")
                    system_safe = False
                continue
            else:
            ...
            if limb.voltage < self.LIMITS["VOLTAGE_MIN"]:
                self.raise_alarm(f"L{limb_id}_UNDERVOLT", ms.SEVERITY_CRITICAL, f"V={limb.voltage}")
                system_safe = False
            else:
            ...
            if limb.temp >= self.LIMITS["TEMP_CRITICAL"]:
                self.raise_alarm(f"L{limb_id}_FIRE", ms.SEVERITY_CRITICAL, f"T={limb.temp}C (CRITICAL)")
                system_safe = False
            ...
            if limb.temp >= self.LIMITS["TEMP_PAUSE"]:
                self.raise_alarm(f"L{limb_id}_OVERHEAT_PAUSE", ms.SEVERITY_PAUSE, f"T={limb.temp}C (PAUSE)")
                limb.needs_cooldown = True
            else:
            ...
            if limb.temp >= self.LIMITS["TEMP_THROTTLE"]:
                if not getattr(limb, 'needs_cooldown', False):
                    self.raise_alarm(f"L{limb_id}_THROTTLE", ms.SEVERITY_WARNING, f"T={limb.temp}C")
            ...
            if total_pkts > self.LIMITS["LQI_BURN_IN_PACKETS"]:
                if lqi < self.LIMITS["LQI_MIN"]:
                    self.raise_alarm(f"L{limb_id}_LQI_LOW", ms.SEVERITY_WARNING, f"LQI={round(lqi,1)}%")
                else:
            ...
            for act_name, act in limb.actuators.items():
                if "FAULT" in act.verification_state:
                    self.raise_alarm(f"L{limb_id}_{act_name}_FAULT", ms.SEVERITY_CRITICAL, act.verification_state)
                    system_safe = False
                else:
    ...
    def acknowledge_alarm(self, code):
        ...
        if code in self.active_alarms:
            ...
            if severity == ms.SEVERITY_CRITICAL:
                logger.warning(f"Safety Override Rejected: Cannot acknowledge CRITICAL alarm {code}.")
                return False
    ...
    def raise_alarm(self, code, severity, context=""):
        ...
        if code not in self.active_alarms and code not in self.acknowledged_alarms:
            ...
            self.history.append({"ts": timestamp, "code": code, "lvl": severity, "ctx": context})
    ...
    def get_active_details(self):
        ...
        """
        data = {k: {"lvl": v, "msg": self.alarm_contexts.get(k, ""), "acked": False} 
                for k, v in self.active_alarms.items()}
        data.update({k: {"lvl": v, "msg": self.alarm_contexts.get(k, ""), "acked": True} 
