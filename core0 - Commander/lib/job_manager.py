...
class JobManager:
    ...
    def __init__(self, coordinator):
        ...
        logger.info(
            "[Job] JobManager Initialized with Atomic Handshake and Calibration logic."
        )
    ...
    async def run_arming_sequence(self, profile_name):
        ...
        if GLOBAL_TWIN.host_state == ms.STATE_ERROR:
            self.coord._notify(
                "Arming Blocked: System in ERROR. Reset required.", type="negative"
            )
            return False

        if not self.coord.safety.is_safe_to_arm():
            self.coord._notify(
                "Arming Blocked: Critical hardware alarms active.", type="negative"
            )
            return False

        if self.active_job:
            self.coord._notify(
                f"Arming Blocked: {self.active_job} in progress.", type="warning"
            )
            return False
        ...
        try:
            ...
            if not success:
                self.coord._notify(
                    "Arming Failed: Hardware failed to acknowledge profile.",
                    type="negative",
                )
            ...
            self.coord._notify(
                f"SUCCESS: System Flowing ({profile_name})", type="positive"
            )
            return True
    ...
    async def run_calibration_sequence(self):
        ...
        if GLOBAL_TWIN.host_state not in [ms.STATE_IDLE, ms.STATE_DEV]:
            self.coord._notify(
                "Calibration Blocked: System must be IDLE or DEV.", type="negative"
            )
            return

        if self.active_job:
            self.coord._notify(
                f"Job Collision: {self.active_job} is running.", type="warning"
            )
            return
        ...
        try:
            ...
            self.coord._notify(
                "Calibration: 5s Kinetic Burst active...", type="warning"
            )

            # Ignition: Drive Distributor Conveyor
            await self.coord.send_physical(
                3, meowprotocol.MSG_TYPE_CMD, "ACT:conveyor=0.5"
            )
            ...
            # Shutdown
            await self.coord.send_physical(
                3, meowprotocol.MSG_TYPE_CMD, "ACT:conveyor=0.0"
            )
            ...
            if pulse_delta < 50:
                raise Exception(
                    f"Insufficient Motion ({pulse_delta}p). Check belt/P2 link."
                )
            ...
            self.coord._notify(
                f"CALIBRATION SUCCESS: {new_ratio:.4f} mm/p", type="positive"
            )
            logger.info(
                f"[Job] Calibration Wizard Success. Delta: {pulse_delta}p, Ratio: {new_ratio:.6f}"
            )
    ...
    async def _dispatch_configs(self, profile):
        ...
        if "vibration_freq" in loader_cfg:
            ...
            seq = await self.coord.send_physical(
                1, meowprotocol.MSG_TYPE_SET_CFG, f"CFG:ACT:feeder:freq={f_val}"
            )
            if seq:
                handshake_list.append((1, seq))

        if "flow_rate" in loader_cfg:
            r_val = loader_cfg["flow_rate"]
            seq = await self.coord.send_physical(
                1, meowprotocol.MSG_TYPE_SET_CFG, f"CFG:ACT:feeder:flow={r_val}"
            )
            if seq:
                handshake_list.append((1, seq))
        ...
        if "belt_speed" in dist_cfg:
            ...
            seq = await self.coord.send_physical(
                3, meowprotocol.MSG_TYPE_SET_CFG, f"CFG:ACT:conveyor:speed={s_val}"
            )
            if seq:
                handshake_list.append((3, seq))
        ...
        while time.time() < timeout_at:
            ...
            for limb in GLOBAL_TWIN.limbs.values():
                ...
                if limb.remote_state not in [ms.STATE_IDLE, ms.STATE_READY]:
                    logger.error(
                        f"[Job] Arming Aborted: Pico {limb.id} is in {limb.remote_state}, expected IDLE."
                    )
                    return False
