# project_mapper.py - Industrial System Auditor (v2.5)
...
# CHANGES v2.5:
#   - FIXED: Expanded regex to support alphanumeric specs (e.g. 4.3.1.1.A).
#   - FIXED: Added support for compound spec tags (e.g. 11.1 & 11.2).
#   - IMPROVED: Enhanced forensic summary formatting.
...
class IndustrialMapper(ast.NodeVisitor):
    ...
    Now supports complex alphanumeric and compound industrial tags.
    """
    def __init__(self, filename):
        self.filename = filename
        self.results = []
        self.current_class = None
        # [v2.5 FIX] Capture everything between [Spec and ] to support 4.3.1.1.A and & logic
        self.spec_pattern = re.compile(r"\[Spec\s+([^\]]+)\]")
    ...
    def get_summary(self, node):
        ...
        if doc:
            ...
            # Extract raw spec strings
            specs = self.spec_pattern.findall(doc)
            
            # Process and clean spec tags
            processed_specs = []
            for s in specs:
                # Handle compound tags like "11.1 & 11.2"
                parts = re.split(r'[&\s,]+', s)
                for p in parts:
                    p = p.strip()
                    if p:
                        processed_specs.append(p)
            
            unique_specs = sorted(list(set(processed_specs)))
    ...
    def visit_FunctionDef(self, node):
        ...
        summary, specs, lines = self.get_summary(node)
        # Skip internal methods without docstrings
